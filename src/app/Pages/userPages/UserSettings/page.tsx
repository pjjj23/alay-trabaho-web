...
const ProfileCard = () => {
  ...
  const [resumeLink, setResumeLink] = useState<string | null>("");
    ...
    useEffect(() => {
      ...
      if (storedUser) {
          ...
          setProfession(userData.profession || "No profession saved");
          ...
          setResumeLink(userData.resumeLink || "");
            ...
            setFormData({
              ...
              profession: userData.profession || "No profession saved",
              birthdate: userData.birthdate || "No birthday saved",
              role: userData.role || "Guest",
              contactNum: userData.contactNumber || "NoContactNumber",
    ...
    const handleSave = async () => {
        ...
        let resumeUrl = tempData.resumeLink;
    
        // ✅ Firebase storage logic for resume file upload
        if (tempData.resumeLink instanceof File) {
            try {
                const file = tempData.resumeLink;
                const trimmed_email_key = userEmail.replace(/[^a-zA-Z0-9]/g, "_");
                const storageRef = ref(storage, `AlayTrabaho/${trimmed_email_key}/resumeLink`);
                const uploadTask = uploadBytesResumable(storageRef, file);
    
                await uploadTask; // Wait for the upload to complete
                resumeUrl = await getDownloadURL(storageRef); // Get the file URL
    
                console.log('File uploaded to Firebase Storage:', resumeUrl);
            } catch (firebaseError) {
                console.error("Firebase Upload Error:", firebaseError);
                setMessage(`Firebase Upload Error: ${firebaseError.message}`);
                setLoading(false);
        ...
        const updateData = {
            ...
            role: tempData.role,
            contactNum: tempData.contactNum,
            userAddress: tempData.userAddress,
            profileDescription: tempData.profileDescription,
            resumeLink: resumeUrl, // Use uploaded URL
        };
    
        try {
            const response = await fetch(
                `https://alaytrabaho-d6g3b8h0gabdgwgb.canadacentral-01.azurewebsites.net/api/applicant/${userEmail}`,
                {
                    method: "PUT",
            ...
            setProfession(updatedUser.profession || "No profession saved");
    ...
    const ProfileField = ({ label, value, field, editable = true }) => {
        const handleChange = (e) => {
            if (field === "resumeLink") {
                ...
                if (file) {
                    setTempData({ ...tempData, resumeLink: file });
                    setResumeLink(file.name); // Display file name as preview
                }
            } else {
                ...
                if (field === "contactNum") setContactNumber(e.target.value);
                if (field === "profession") setProfession(e.target.value);
                if (field === "userAddress") setUserAddress(e.target.value);
                if (field === "profileDescription") setProfileDescription(e.target.value);
                if (field === "resumeLink") setResumeLink(e.target.value);
            }
        };
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <ProfileField
                                        ...
                                        editable={isEditing}
                                    />
                                    <ProfileField label="Resume Link" field="resumeLink" value={formData.resumeLink} />
                                </div>
                            </div>
                        </div>
                    </div>
